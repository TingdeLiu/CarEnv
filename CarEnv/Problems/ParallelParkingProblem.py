...
class ParkingProblem(Problem):
    def __init__(self, env, k_continuous=.05, max_time=None, soft_collisions=False, reward_term="lffds", reward_norm_x=20.,
                 reward_norm_y=5., reward_norm_theta=np.pi / 2):
        super(ParkingProblem, self).__init__(env)

        self.target_pose = None
        self.k_continuous = k_continuous
        self.max_time = max_time or 15.
        self.reward_term = reward_term
        self.help_pos = None
        self.track_dict = None
        self.soft_collisions = soft_collisions
        self.reward_norm_x = reward_norm_x
        self.reward_norm_y = reward_norm_y
        self.reward_norm_theta = reward_norm_theta
        self._track_renderer = None

    def transform_ctx(self, ctx, width, height):
        from ..Rendering.BirdView import transform_for_pose
        transform_for_pose(ctx, width, height, self.env.ego_pose, orient_forward=False, scale=30.)
    ...
    def observe_state(self):
        return np.array([
            ...
            np.clip(self.env.vehicle_model.velocity[0] / 15., -1., 1.),
            self.env.vehicle_model.steering_angle / self.env.vehicle_model.steering_model.beta_max,
        ], dtype=np.float32)

    def _make_problem(self, env, rng: np.random.Generator):
    ...
    def configure_env(self, rng=None) -> Tuple[float, float, float]:
        ...
        start_pose, target_pose, cones_pos, track_dict = self._make_problem(self.env, rng)
        ...
        self.env.add_object('cones', BatchedCones(cones, soft_collision_distance=.15 if self.soft_collisions else None))
    ...
    def pose_reward(self, current_pose, target_pose) -> PosRewardInfo:
        ...
        if self.reward_term == "lffds":
            reward_x = np.maximum(0., self.reward_norm_x - abs_delta_x) / self.reward_norm_x
            reward_y = np.maximum(0., self.reward_norm_y - abs_delta_y) / self.reward_norm_y
            reward_angle = np.maximum(0., np.cos(delta_angle))
            reward = reward_x * reward_y * reward_angle
        elif self.reward_term == "lffds2":
            reward_x = np.maximum(0., self.reward_norm_x - abs_delta_x) / self.reward_norm_x
            reward_y = np.maximum(0., self.reward_norm_y - abs_delta_y) / self.reward_norm_y
            reward_angle = np.maximum(0., self.reward_norm_theta - np.abs(delta_angle)) / self.reward_norm_theta
            reward = reward_x * reward_y * reward_angle
    ...
    def is_out_of_bounds(self, env):
        return not self.track_dict['poly'].contains(env.ego_collider)
    ...
    def update(self, dt: float) -> Tuple[bool, bool]:
        ...
        if self.is_out_of_bounds(env):
            env.set_reward(-1)
            return True, False

        reward_info = self.pose_reward(pose, self.target_pose)

        env.get_or_create_metric('AbsDeltaX', CounterMetric).set(abs(reward_info.delta_x))
        env.get_or_create_metric('AbsDeltaY', CounterMetric).set(abs(reward_info.delta_y))
        env.get_or_create_metric('AbsDeltaAngle', CounterMetric).set(abs(reward_info.delta_angle))
        env.get_or_create_metric('PoseReward', CounterMetric).set(reward_info.reward)
...
class ParallelParkingProblem(ParkingProblem):
    def __init__(self, env, max_time=None, start="before", **kwargs):
        if max_time is None:
            max_time = 15. if start == 'before' else 7.

        super(ParallelParkingProblem, self).__init__(env, max_time=max_time, **kwargs)
        self.start = start
    ...
    def _make_problem(self, env, rng=None):
        ...
        track_dict = {
            ...
            'poly': Polygon(np.array([[-25 - street_pad, -street_width / 2. - street_pad],
                                      ...
                                      [-25. + gap_start + gap_size + street_pad,
